                ...
                public void onComplete(Task<DataSnapshot> task) {
                    if (!task.isSuccessful()) {
                        ...
                        if (task.getResult().child("collect_id").child(plusid).getValue() != null) {//代表之前這個uid收藏了這則貼文
                            Glide.with(Pushlist.this).load(R.drawable.heart_popupcolor).into(heart_popup);
                        } else {//未收藏
                            Glide.with(Pushlist.this).load(R.drawable.heart_popup).into(heart_popup);
                        }
                    }
                }
            });
                ...
                public void onComplete(Task<DataSnapshot> task) {
                    if (!task.isSuccessful()) {
                        ...
                        if (task.getResult().child("baduid").child(uid).getValue() != null) {//這個uid之前已按過倒讚了
                            Glide.with(Pushlist.this).load(R.drawable.good_bold).into(good_bold);
                            Glide.with(Pushlist.this).load(R.drawable.bad_color).into(bad_bold);
                        } else if (task.getResult().child("gooduid").child(uid).getValue() != null) {  //按讚
                            Glide.with(Pushlist.this).load(R.drawable.good_color).into(good_bold);
                            Glide.with(Pushlist.this).load(R.drawable.bad_bold).into(bad_bold);
                        } else {    //沒有按過讚&倒讚
                            Glide.with(Pushlist.this).load(R.drawable.bad_bold).into(bad_bold);
                            Glide.with(Pushlist.this).load(R.drawable.good_bold).into(good_bold);
                        }
            ...
            public void onComplete(Task<DataSnapshot> task) {
                if (!task.isSuccessful()) {
                    ...
                    if(task.getResult().child("gooduid").child(uid).getValue() != null){//這個uid之前已按過讚了
                        Glide.with(Pushlist.this).load(R.drawable.good_bold).into(good_bold);  //收回之前按讚的
                        //要在good那邊-1和移除uid
                        myRef.child("push").child(plusid).child("gooduid").child(uid).removeValue();
    ...
    private void firebase_select(DatabaseReference db) {
        ...
        db.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                Collections.reverse(items); //1026珵 新增
                if(plus.size()>0) {
